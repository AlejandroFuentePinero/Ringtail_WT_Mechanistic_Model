...
for(s in 1:nspecies){ # start loop for each ringtail species
  ...
  for(i in 1:nsites){
    ...
    if(sim){ # turn on/off above
      ...
      save(micro, file = paste0('model_output/micro_', sites[i, 1], '_1990_2022.Rda')) # save microclimate
    ...
    shademet <- as.data.frame(micro$shadmet)
    ...
    TSKYs = (metout$TSKYC * 0.7) + (shademet$TSKYC * 0.3) # sky temperature (°C)
    ...
    treg$site <- sites[i,1]
    treg$species <- spp
    write_csv(treg, paste0('model_output/output_model_2_possum/treg_', sites[i, 1],"_", spp, '.csv')) # save output ####!!!!!!!##### modify path here
    ...
    morph$species <- spp
    write_csv(morph, paste0('model_output/output_model_2_possum/morph_', sites[i, 1],"_", spp, '.csv')) # save output ####!!!!!!!##### modify path here
    ...
    enbal$species <- spp
    write_csv(enbal, paste0('model_output/output_model_2_possum/enbal_', sites[i, 1],"_", spp, '.csv')) # save output ####!!!!!!!##### modify path here
    ...
    masbal$species <- spp
    write_csv(masbal, paste0('model_output/output_model_2_possum/masbal_', sites[i, 1],"_", spp, '.csv')) # save output ####!!!!!!!##### modify path here
