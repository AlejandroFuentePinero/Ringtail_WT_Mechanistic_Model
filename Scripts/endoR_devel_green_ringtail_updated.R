endoR_devel_grt <-
            ...
            RESPIRE = 1)
  {
    ...
    if (SHAPE_B_MAX < SHAPE_B) {
      message(
        "warning: SHAPE_B_MAX must greater than than or equal to SHAPE_B, resetting to SHAPE_B \n"
      )
      SHAPE_B_MAX <- SHAPE_B
    }
    if (PANT_INC == 0) {
    ...
    while (QGEN < QBASAL) {
      IRPROP.out <- IRPROP((0.7 * TS + 0.3 * TFA),
                           DHAIRD,
                           DHAIRV,
                           LHAIRD,
                           LHAIRV,
      ...
      for (S in 1:2) {
        ...
        SIMULSOL.out[S,] <- SIMULSOL(DIFTOL,
                                     IPT,
                                     FURVARS,
                                     GEOMVARS,
                                     ENVVARS,
    ...
    masbal1 <- c(AIRVOL,
                 ...
                 AIRML1,
                 AIRML2) * 3600
    treg <- matrix(data = treg1,
                   nrow = 1,
                   ncol = 15)
    morph <- matrix(data = morph1,
                    nrow = 1,
                    ncol = 20)
    masbal <- matrix(data = masbal1,
                     nrow = 1,
                     ncol = 10)
    enbal <- matrix(data = enbal1,
                    nrow = 1,
                    ncol = 10)
    treg.names <- c(
      "TC",
    ...
    if (max(treg) == 0) {
      warning(
        "A solution could not be found and panting/'sweating' options are exhausted; try allowing greater evaporation or allowing higher body maximum body temperature"
      )
    }
    endo.out <- list(
      treg = treg,
