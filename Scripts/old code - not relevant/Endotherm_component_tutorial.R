###################################################################
## NicheMapR - Endotherm Model tutorial II: components functions ##
## Used with Green Ringtail Possum data                          ##
###################################################################
...
setwd("~/Library/CloudStorage/OneDrive-JamesCookUniversity/Ringtail - Mechanistic model - Wet Tropics")
...
# environmental input
TA <- 26  # air temperature, for calculation of conductivity of air (°C)

# shape input
PVEN <- fur[[20,6]]  # maximum fraction of surface area that is ventral (fractional, 0-1)
...
RHOD <- fur[[14,6]]  # hair density, dorsal (1/m2) 
RHOV <- fur[[16,6]]  # hair density, ventral (1/m2)
REFLD <- 0.301  # fur reflectivity dorsal (fractional, 0-1) 
REFLV <- 0.301  # fur reflectivity ventral (fractional, 0-1)
#KHAIR <- fur[[27,6]]  # hair thermal conductivity (W/m°C)
...
KRAD = (16.0*SIG*(TRAPPX+273.15)^3)/(3*BETARA[1])
...
# GEOM_ENDO subroutine ----------------------------------------------------

## Computes aspects of the animal geometry. It required outputs from IRPROP for fur properties

#input
AMASS <- fur[[23,6]]  # animal mass (kg)
ANDENS <- AMASS*100  # animal density (kg/m3)
...
RELHUM <- 80  # relative humidity (%)
TC <- fur[[28,6]]  # core temperature (°C)
TSKIN <- 32  # skin temperature (°C)
...
FAVEG <- 0.7  # this is for overhead veg (at TAREF) (-)
FABUSH <- 0.9  # this is for veg below/around animal (at TALOC) (-)
...
FURTHRMK <- fur[[27,6]]  # user-specified fur thermal conductivity (W/m C), not used if 0
SHAPE <- 4  # shape, 1 is cylinder, 2 is sphere, 3 is plate, 4 is ellipsoid
...
colnames(SIMULSOL.out) <- c("SIDE", "TFA", "TSKIN", "QCONV",
                            ...
                            "QRVEG", "QRGRD", "QFSEVAP", "NTRY", "SUCCESS", "TRUE THERMAL CONDUCTANCE???") ## ERROR HERE //!!!\\\
