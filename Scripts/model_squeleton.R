...
for(s in 1:nspecies){ # start loop for each ringtail species
  ...
  for(i in 1:nsites){
    ...
    if(sim){ # turn on/off above
      ...
      TAs <- metout$TALOC # air temperature at local height (°C)
      TAREFs <- metout$TAREF # air temperature at reference height (°C)          
      TGRDs = soil$D0cm # ground temperature (°C)                               # (CHECK THIS)
      TSKYs = metout$TSKYC # sky temperature (°C)
      QSOLRs = metout$SOLR # solar radiation, horizontal plane (W/m2)
      Zs = metout$ZEN # zenith angle of sun (degrees from overhead)
      ELEV = micro$elev # elevation (m)
      ABSSB = 0.8 # solar absorptivity of substrate (fractional, 0-1)           # (CHECK THIS)
      RHs <- metout$RH # relative humidity (%)
      VELs <- metout$VLOC # wind speed (m/s)
      ...
      treg$site <- sites[i,1]
      treg$species <- spp
      write_csv(treg, paste0('model_output/treg_', sites[i, 1],"_", spp, '.csv')) # save output
      ...
      morph$species <- spp
      write_csv(morph, paste0('model_output/morph_', sites[i, 1],"_", spp, '.csv')) # save output
      ...
      enbal$species <- spp
      write_csv(enbal, paste0('model_output/enbal_', sites[i, 1],"_", spp, '.csv')) # save output
      ...
      masbal$species <- spp
      write_csv(masbal, paste0('model_output/masbal_', sites[i, 1],"_", spp, '.csv')) # save output
