...
depth <- read_csv("Data/data_input/fur_depth_grtp.csv")

depth <- depth %>% group_by(fur_id, side) %>% summarise(depth = mean(depth.m))

##################################################### In previous estimations, I
#used the fur depth averaged across sides (dorsal, ventral, and flank) to
#calculate thermal conductance. The result looked unrealistic for the ventral
#conductivity. It was too low. Lower than theoretically possible. A possible
#solution was to account for intra-variability across pelts. This was done by
#using the average depth (n = 6 measurements) for each individual pelt and side
#(ventral, dorsal, flank). This is a more logical approach than assuming a
#constant depth. Especially since thermal conductance is sensitive to depth.
#After account for this, the results look very similar. This is because the
#variability across pelts is not great, so we still get a very low value of
#thermal conductance (>0.025). Next, I considered human error. Did I induce bias
#on some of the measurements taken on the ventral part of the fur? This part is
#tricky to measure. This would be especially easy for the ventral area of the
#fur, because it is very short. A slight push with the caliper would mean double
#or triple the measure you would get. Thus, the sensitivity of manually
#measuring fur depth on the ventral part would be susceptible to human error. I
#consider this possibility by increasing the depth of the pelts with the lowest
#value by a error rate. I increased the error until the values of the ventral
#thermal conductance was closer, but below, the flank observed measurements
#(which would be the logical value they should get). I then compared this new
#observed measure to the effective thermal conductance simulated with NicheMapR,
#with the idea that the values should be higher than KEFF estimations. I would
#have to have induced an error across the measurements of 70-100% to have a
#value close to the expected reality (100% to be above the KEFF line).
...
ins <- read_csv("Data/data_input/fur_insulation_clean.csv") 
...
##Note that Tfur_base is much higher than target water temp
# and fur tip temp & air temp are often very similar, especially for ventral fur

ggplot(data = ins[ins$`Target wind speed (m/s)`==0,], aes(x=`Fur ID`, y=`Fur base temp (deg C)/Ts`)) + 
         geom_point() + 
         facet_grid(.~`Fur region code`) + 
  geom_point(aes(x=`Fur ID`,y=`Fur tip temp (deg C)`), col="orange", pch = 5) + 
  geom_point(aes(x=`Fur ID`,y=`Air temp (deg C)/Ta`), col="blue", alpha= 0.2)
...
#################################################################################
## Part II - calculate observed thermal conductance per individual & windspeed ##
#################################################################################
...
TAs <- summary_thermal_cond$T_air[summary_thermal_cond$wind==0 & summary_thermal_cond$side!="flank"]

TSKIN <- summary_thermal_cond$T_base[summary_thermal_cond$wind==0 & summary_thermal_cond$side!="flank"] # TSKIN = T_BASE

TFA <- summary_thermal_cond$T_tip[summary_thermal_cond$wind==0 & summary_thermal_cond$side!="flank"] # TFA = T_TIP

SIDES<-summary_thermal_cond$side[summary_thermal_cond$wind==0 & summary_thermal_cond$side!="flank"]
  
IDs<-summary_thermal_cond$fur_id[summary_thermal_cond$wind==0 & summary_thermal_cond$side!="flank"]
...
for(t in 1:length(TAs)){
  #for(side in 1:3){ # 1 = mean, 2 = dorsal, 3 = ventral
    ...
    # shape input
    if(side=="belly"){
    PVEN <- 1  # maximum fraction of surface area that is ventral (fractional, 0-1)
    }else{
    PVEN <- 0
    }
    ...
    # output
    KEFARA <- IRPROP.out[1]  # effective thermal conductivity of fur array, mean, dorsal, ventral (W/mK)
    BETARA <- IRPROP.out[4]  # term involved in computing optical thickness (1/m)
...
# Calculate KRAD for each measurement
    ...
    KRAD = (16.0*SIG*(TRAPPX+273.15)^3)/(3*BETARA)
    ...
    vec <- c(side, IDs[t], TA, TSKIN[t], TFA[t] ,KEFF, KRAD, KFUR)
...
# 
# 
# obs <- summary_thermal_cond %>% filter(wind == 0)
...
pred$fur_id<-as.numeric(pred$fur_id)
pred$KFUR<-as.numeric(pred$KFUR)
pred$KEFF<-as.numeric(pred$KEFF)
pred$KRAD<-as.numeric(pred$KRAD)

ggplot(data=obs, aes(x=fur_id, y=thermal_cond_bt_m)) + 
  geom_point() + 
  facet_grid(.~side) + 
geom_point(data=pred, aes(x=as.numeric(fur_id), y=KFUR), col="red") + 
  geom_point(data=pred, aes(x=as.numeric(fur_id), y=KEFF), col="blue") + 
  geom_point(data=pred, aes(x=as.numeric(fur_id), y=KRAD), col="green")
...
#   geom_point(aes(y = thermal_cond_bt_m), label = "BA", hjust = 1)+
#   labs(x = "Air Temperature (°C)", y = "Thermal conductance (W/m°C)")+
