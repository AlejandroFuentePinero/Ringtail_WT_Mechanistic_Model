endoR_devel_grtp <- function (TA = 20, TAREF = TA, TGRD = TA, TSKY = TA, VEL = 0.1, 
          ...
          RELXIT = 100, QBASAL = (70 * AMASS^0.75) * (4.185/(24 * 3.6)), 
          RQ = 0.8, EXTREF = 20, PANT_MAX = 5, Q10 = 2, TS = TC - 3, 
          TFA = TA, DIFTOL = 0.001, THERMOREG = 1, RESPIRE = 1) 
{
  ...
  while (QGEN < QBASAL) {
    ...
    RHOARA <- RHOAR[1]
    ZFUR <- ZZFUR[1]
    GEOM.out <- GEOM_ENDO(AMASS, ANDENS, FATPCT, SHAPE, ZFUR, 
                          SUBQFAT, SHAPE_B, SHAPE_C, DHARA, RHOARA, PCOND, 
                          SAMODE, ORIENT, Z)
    VOL <- GEOM.out[1]
    D <- GEOM.out[2]
    MASFAT <- GEOM.out[3]
    ...
    for (S in 1:2) {
      ...
      FURVARS <- c(LEN, ZFUR, FURTHRMK, KEFF, BETARA, FURTST, 
                   ZL, LHAR[S + 1], DHAR[S + 1], RHOAR[S + 1], REFLFR[S + 
                                                                        1], KHAIR, S)
